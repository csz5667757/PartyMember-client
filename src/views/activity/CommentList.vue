<template>
	<div id='list'>
		...
		<el-table :data="pageInfo.list" style="width: 100%" stripe @selection-change="handleSelectionChange">
			...
			<el-table-column label="活动名称" prop="activityTitle" sortable></el-table-column>
			...
			<!-- 单选框value={1,2} -->
			<el-table-column label="评论内容" prop="content" width="700px" header-align="center" align="center"
		...
		methods: {
			...
			notShow() {
				if (this.multipleSelection.length < 1) {
					...
					this.$http.post("/comment/notshow", {
						'ids': ids
					}, true).then(
						(data) => {
							if (data > 0) {
