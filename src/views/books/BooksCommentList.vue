<template>
	<div id='list'>
		...
		<el-table :data="pageInfo.list" style="width: 100%"  stripe
		 @selection-change="handleSelectionChange">
			...
			<el-table-column label="图书名称" prop="seriesName" sortable></el-table-column>
			<el-table-column label="评论人" prop="nickName" sortable></el-table-column>
			...
			<!-- 单选框value={1,2} -->
			<el-table-column label="评论" prop="content" width="700px" header-align="center" align="center" show-overflow-tooltip></el-table-column>
...
<script>
	export default {
		...
		methods: {
			getList() {
				this.$http.get("/books/comment/select", {
			...
			del(item) {
				this.$mm.delete(() => {
					this.$http.post("/books/comment/delete", {
						'id': item.id
					}, true).then(
						(data) => {
							if (data > 0) {
			...
			show() {
				if (this.multipleSelection.length < 1) {
					...
					this.$http.post("/books/comment/show", {
						'ids': ids
					}, true).then(
						(data) => {
							if (data > 0) {
			...
			notShow() {
				if (this.multipleSelection.length < 1) {
					...
					this.$http.post("/books/comment/notshow", {
						'ids': ids
					}, true).then(
						(data) => {
							if (data > 0) {
