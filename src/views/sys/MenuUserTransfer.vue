<template>
	<div>
		<el-dialog :close-on-click-modal="false" title="菜单配置" :visible.sync="visible" width="30%" :before-close="handleClose">
		  <span>
			  <el-tree :data="treeData" show-checkbox default-expand-all node-key="id" ref="tree" :check-strictly="false" highlight-current :props="defaultProps"></el-tree>
		  </span>
...
export default {
	...
	methods:{
		...
		dialogOkClick(){
			...
			this.$http.post("/sys/setmenuauthbyuserid",{userId:this.rowId,authId:this.$refs.tree.getCheckedKeys()},true).then(res=>{
				console.log(res);
